...
class PersonaController extends Controller {
    ...
    public function importarPersonasCsvAction(Request $request) {
        ...
        return $this->render('PersonasBundle:Persona:importarPersonasCsv.html.twig', array(
                    'form' => $form->createView(),
        ));
    }
...
//                    $numero_documento = $datos[12];
                    if (is_numeric($datos[12])) {
                        ...
                        if ($persona) {
                            ...
                            $etiquetaOriginal = $em->getRepository('PersonasBundle:PersonaEtiqueta')->findOneBy(array('persona' => $persona->getId(), 'etiqueta' => $etiqueta->getId()));
                            if (!$etiquetaOriginal) {
                                //agrego la etiqueta
                                $personaEtiqueta = new PersonaEtiqueta();
                                $personaEtiqueta->setPersona($persona);
                            ...
                            if ($fechaNacimiento) {// \DateTime::createFromFormat("d/m/Y H:i","25/04/2015 15:00");
                                $fechaNacimiento = \DateTime::createFromFormat('Y-m-d H:i:s',$fechaNacimiento." 00:00:00");
                ...
                $this->get('session')->getFlashBag()->add(
                        'error', 'Error en lectura de archivo csv'
                );
            }
        } else {
            $this->get('session')->getFlashBag()->add(
